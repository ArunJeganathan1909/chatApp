...
const Chat = () => {
    ...
    const sendMessage = (e, file = null) => {
        ...
        const newMessage = {
            text: newMessageText,
            isOur: true, // Assuming it's our message since we're sending it
            sender: id,
            recipient: selectedUserId,
            _id: Date.now()
        };
    ...
    useEffect(() => {
        axios.get('/people').then(respose => {
            const offlinePeopleArr = respose.data
                .filter(p => p._id !== id)
        ...
        <div className='flex h-screen'>
            <div className='bg-blue-50 w-1/3 flex flex-col '>
                ...
                <div className='p-2 text-center flex items-center justify-center'>
                    ...
                    <button
                        className='text-sm text-gray-700 bg-blue-300 py-1 px-2 border rounded-lg '
                        onClick={logout}
                    >
                        Logout
                    </button>
            ...
            <div className='flex flex-col bg-blue-100 w-2/3 p-2'>
                            ...
                            <div className='overflow-y-scroll absolute top-0 left-0 right-0 bottom-2'>
                                {messagesWithoutDupes.map((message, index) => (
                                    ...
                                    >
                                        <div
                                            key={index}
                                            className={'p-2 my-2 rounded-2xl inline-block ' + (message.sender === id ? 'text-right bg-blue-500 text-white' : 'bg-white text-gray-500')}
                                        >
                                            {message.text}
                                            {message.file && (
                                                <div>
                ...
                {!!selectedUserId && (
                    <form className='flex gap-2 mx-2' onSubmit={sendMessage}>
                        ...
                        />
                        <label className='bg-blue-200 p-2 text-gray-600 rounded-lg border border-blue-200 cursor-pointer'>
                            <input
                                type="file"
                                className='hidden'
                                onChange={sendFile}
